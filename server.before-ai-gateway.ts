...
type LicensePlan = "none" | "trial" | "starter" | "monthly" | "lifetime";
type LocalLicense = {
  key?: string;
...
function remoteLicenseToLocal(current: LocalLicense, payload: any, key?: string): LocalLicense {
  ...
  return {
    ...
    quotas: { voice: remote.voice_credits ?? current.quotas.voice ?? 0, image: remote.image_credits ?? current.quotas.image ?? 0, gemini: trial ? (current.quotas.gemini || 2000) : 0 },
  };
}
...
app.get("/api/license/status", async (_req, res) => {
  ...
  } catch { return res.status(503).json({ ...publicLicense(emptyLicense()), error: "LICENSE_SERVER_UNAVAILABLE" }); }
});
...
app.post("/api/license/authorize-auto", async (_req, res) => {
  const current = readLicense();
  ...
  try {
    ...
    if (updated.plan === "starter") return res.status(403).json({ allowed: false, plan: updated.plan, error: "Gói Khởi đầu chỉ hỗ trợ Tạo video từng bước. Hãy nâng cấp Gói Pro để dùng Tạo video tự động." });
    return res.json({ allowed: true, plan: updated.plan });
  } catch {
...
app.post("/api/license/trial", async (_req, res) => {
  const current = readLicense();
  try {
    const response = await fetch(`${licenseApiUrl}/trial`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ machine_hash: machineHash, app_version: "1.0.0" }) });
    const payload: any = await response.json();
    if (!response.ok) return res.status(response.status).json({ ...publicLicense(current), error: payload?.error || "TRIAL_ACTIVATION_FAILED" });
    const trial = remoteLicenseToLocal({ ...emptyLicense(), quotas: { voice: 2000, image: 100, gemini: 2000 } }, payload);
...
app.post("/api/license/activate", async (req, res) => {
  ...
  try {
    const response = await fetch(`${licenseApiUrl}/activate`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ activation_key: key, machine_hash: machineHash, app_version: "1.0.0" }) });
    const payload: any = await response.json();
