...
# 1. Replace the entire uploadReferenceImage for Google Labs
  ...
  try {
    ...
    if (platform === 'gemini') {
        ...
        if (isUploadClicked) {
          ...
          // RE-CLICK ATTACH BUTTON (+) to open library to choose the uploaded image
...
# 2. Fix model dropdown popover triggers and count selectors to match EXACT HTML
# Look at the old block:
...
# Replace count select to locate button tabs matching class name / trigger name or role tab
content = content.replace(
...
# Replace ratio select to locate button tabs matching class name / trigger name or role tab
content = content.replace(
